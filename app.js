    ...
    openBasketBtn.onclick = () => {
        basketSidebar.style.transform = 'translateX(0)';
    };

    closeBasketBtn.onclick = () => {
        basketSidebar.style.transform = 'translateX(100%)';
    };
...
function addToBasket(title, price, image) {
    if (basketElement[title]) {
        basketElement[title].count++;
    } else {
        basketElement[title] = { price, image, count: 1 };
    } updateBasket(); 
}
...
function updateBasket() {
    ...
    Object.keys(basketElement).forEach(title => {
        const { price, image, count } = basketElement[title];

        basketContent.innerHTML += `
            <div class="bg-gray-200 p-2 rounded my-2 flex justify-between items-center">
                <div class="flex items-center">
                    <img src="${image}" alt="${title}" class="w-16 h-16 object-cover rounded mr-2">
                    <div>
                        <h3 class="font-semibold">${title}</h3>
                        <p>${price}</p>
                        <p class="text-orange-700">sayı : ${count}</p>        
                    </div>
                </div>
            </div>
        `;
